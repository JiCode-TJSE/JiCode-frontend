  ...
  <div class="container">
    ...
    <el-card class="item-card">
      ...
      <el-row :gutter="20" class="search-and-filter">
        <el-col :span="8">
          <el-input placeholder="搜索 (Ctrl+G)" prefix-icon="el-icon-search" v-model="searchText" class="search-input">
          </el-input>
        </el-col>
        <el-col :span="5">
      ...
      <div v-for="item in filteredItems" :key="item.id" class="list-item">
        <el-tag :type="typeColor">{{ item.prefix  }}</el-tag>
...
import { getProductRequest,getUserInfoRequest } from '@/api/account.js'
import { getJoinInfoRequest } from '@/api/account.js'
...
export default {
  setup() {
    ...
    const fetchItems = async () => {
      try {
        // 获取第一个接口的数据
        const response = await getJoinInfoRequest("1");//localStorage.getItem("accountId")
        let allItems = []; // 用于存储合并后的数据

        if (response.code === 200 && Array.isArray(response.data)) {
          const itemsFromResponse = response.data.map(item => ({
            type: item.type,
        ...
        // 获取第二个接口的数据
        const response1 = await getProductRequest("1");//organizationId.value

        if (response1.code === 200 && Array.isArray(response1.data.showingDatas)) {
          const itemsFromResponse1 = response1.data.showingDatas.map(item => ({
            type: item.type,
            typeColor: getTypeColor(item.type),
            prefix: getTypeCode(item.type),
            name: item.item_name,
    ...
    onMounted(async () => {
      fetchItems
      try {
        const response = await getUserInfoRequest("1");//localStorage.getItem("accountId")
...
<style scoped>
.primary { color: #409EFF; } /* Element UI 的 primary 色 */
.success { color: #67C23A; } /* Element UI 的 success 色 */
.warning { color: #E6A23C; } /* Element UI 的 warning 色 */
.info { color: #909399; } /* Element UI 的 info 色 */
